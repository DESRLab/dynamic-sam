...
__all__ = ['HM3D', 'HM3DConfig']

@dataclass(frozen=True)
class HM3DConfig(DSConfig):
    quantization: float
    num_sampling_points: int

class HM3D(PointCloudBaseDataset[HM3DConfig]):
    IGNORE_CLASS_ID = 0
    NAME = 'HM3D'
    
    def __init__(self, split: Split, config: HM3DConfig) -> None:
        super().__init__(split, config)
        
    def setup_files(self, split: Split, config: HM3DConfig) -> None:
