...
__all__ = ['ARKit', 'ARKitConfig']

@dataclass(frozen=True)
class ARKitConfig(DSConfig):
    quantization: float
    num_sampling_points: int

class ARKit(PointCloudBaseDataset[ARKitConfig]):
    IGNORE_CLASS_ID = 0
    NAME = 'ARKit'
    
    def __init__(self, split: Split, config: ARKitConfig) -> None:
        super().__init__(split, config)
        
    def setup_files(self, split: Split, config: ARKitConfig) -> None:
