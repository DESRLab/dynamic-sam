...
__all__ = ['MultiScan', 'MultiScanConfig']

@dataclass(frozen=True)
class MultiScanConfig(DSConfig):
    quantization: float
    num_sampling_points: int

class MultiScan(PointCloudBaseDataset[MultiScanConfig]):
    IGNORE_CLASS_ID = 0
    NAME = 'MultiScan'

    def __init__(self, split: Split, config: MultiScanConfig) -> None:
        super().__init__(split, config)
        
    def setup_files(self, split: Split, config: MultiScanConfig) -> None:
