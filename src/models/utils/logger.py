...
def get_root_logger(log_file=None, log_level=logging.INFO, name='main'):
    ...
    logging_filter.filter = lambda record: record.find(name) != -1 # type: ignore
...
def get_logger(name, log_file=None, log_level=logging.INFO, file_mode='w'):
    ...
    if name in logger_initialized: # type: ignore
        return logger
    # handle hierarchical names
    # e.g., logger "a" is initialized, then logger "a.b" will skip the
    # initialization since it is a child of "a".
    for logger_name in logger_initialized: # type: ignore
    ...
    logger_initialized[name] = True # type: ignore
...
def _strip_prefix_if_present(state_dict: Dict[str, Any], prefix: str) -> None:
    ...
    try:
        metadata = state_dict._metadata  # pyre-ignore # type: ignore
